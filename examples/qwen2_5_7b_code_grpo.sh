...
ACTOR_GLOBAL_BATCH_SIZE="${ACTOR_GLOBAL_BATCH_SIZE:-64}"
...
VAL_GENERATIONS_TO_LOG="${VAL_GENERATIONS_TO_LOG:-1}"
ROLLOUT_N="${ROLLOUT_N:-4}"
DISABLE_EXTRA_VALIDATIONS="${DISABLE_EXTRA_VALIDATIONS:-0}"
...
# EXTRA_ARGS=("trainer.val_before_train=false" "worker.rollout.n=2" "evaluations=[]")
# Or pass them at runtime via:
# EXTRA_TRAIN_ARGS="trainer.val_before_train=false worker.rollout.n=2 evaluations=[]"
# If you want to resume from a specific checkpoint, set:
# LOAD_CHECKPOINT_PATH="/path/to/.../global_step_XX"
EXTRA_ARGS=()

if [[ -n "${EXTRA_TRAIN_ARGS}" ]]; then
    read -r -a EXTRA_ARGS_FROM_ENV <<< "${EXTRA_TRAIN_ARGS}"
...
[[ -n "${VAL_BEFORE_TRAIN}" ]] && TRAIN_ARGS+=("trainer.val_before_train=${VAL_BEFORE_TRAIN}")
[[ -n "${VAL_GENERATIONS_TO_LOG}" ]] && TRAIN_ARGS+=("trainer.val_generations_to_log=${VAL_GENERATIONS_TO_LOG}")
[[ -n "${ROLLOUT_N}" ]] && TRAIN_ARGS+=("worker.rollout.n=${ROLLOUT_N}")
[[ "${DISABLE_EXTRA_VALIDATIONS}" == "1" ]] && TRAIN_ARGS+=("evaluations=[]")
...
CODE_REWARD_WORKERS="${CODE_REWARD_WORKERS}" \
CODE_REWARD_MAX_TESTS="${CODE_REWARD_MAX_TESTS}" \
